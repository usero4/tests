...
def generate_book_title(prompt: str):
    ...
    """
    response = st.session_state.gemini.chat.completions.create(
...
def generate_section(prompt: str, additional_instructions: str):
    ...
    for chunk in response:
        ...
        if tokens:
            yield tokens
        if x_gemini := chunk.x_gemini:
            if not x_gemini.usage:
                continue
            usage = x_gemini.usage
            statistics_to_return = GenerationStatistics(
                input_time=usage.prompt_time,
                output_time=usage.completion_time,
                input_tokens=usage.prompt_tokens,
...
st.write(
    """
# GeminiBook: Write full books using Gemini (1.5 Pro) on Google AI
"""
)
    ...
    with st.form("gemini_form"):
        if not GEMINI_API_KEY:
            gemini_input_key = st.text_input(
                "Enter your Gemini API Key (gsk_yA...):", "", type="password"
            )
        ...
        if submitted:
            ...
            if not GEMINI_API_KEY:
                genai.configure(api_key=gemini_input_key)
                st.session_state.gemini = genai
